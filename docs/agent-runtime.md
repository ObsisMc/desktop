...
- The two ways that fails are classified apart, because the user can only fix one of them without restarting Ora. A CLI absent from this machine reports `agent_not_installed` (`-32001`), stays retryable, and is not logged per attempt, so installing it later is enough to bring the agent up. A package whose bundled executable will not resolve or run reports `agent_unusable` (`-32002`): it fails identically on every attempt, so that agent is abandoned for the rest of the process instead of being retried quietly behind a missing-CLI report. Only that second report carries the plugin's own message through, because giving up makes it the only diagnostic the failure ever produces and only the plugin knows which of its programs failed.
...
- `startSession` is the sole creation path. It opens a temporary setup-registration window, calls `session/new` with the current Session MCP Snapshot, applies the caller's optional model intent against the configuration reported by that handshake, persists the Ora Session, opens its history, and installs its actor before returning. The route channel opened for setup is handed directly to the actor, so non-command setup notifications are not lost.
- A provider session created by `startSession` remains guarded until persistence and actor registration finish. Any error or cancelled command future before that point releases the unexposed provider session with `session/delete` when supported and `session/close` otherwise; failed persistence also removes any history file and soft-deletes a row that was already inserted.
- Load reads Ora's own record and touches no agent. It creates no provider session, registers no actor, and moves no lifecycle state, so a conversation opens whether or not the agent that produced it is still installed or startable; what the client receives is that record, including typed notices for known holes rather than a transcript that silently appears continuous. A session with no history file yet opens as an empty conversation rather than a failure, and a record that cannot be read is reported and degrades the session rather than being shown as an empty one. If an actor already exists it answers its own load, because only it knows the durable cutoff and the records of a turn still in flight: with a prompt in progress it replays the record through that attachment point and then follows later events, neither starting a second prompt nor taking ownership of the first. See [Session History](#session-history).
- A prompt is what acquires a provider session. A session holding no channel attaches first: it registers a route on the current connection generation and calls `session/load` with the private `agentSessionId` and the current Session MCP Snapshot, drains and discards whatever the agent recites, and marks the row Running. Configuration options and available commands reported by that handshake reach the client as session updates ahead of the turn, and are not recorded — they describe the provider serving the conversation, not what was said in it. Admission waits on the attach and on the Session MCP check that follows it, so a send that cannot reach an agent, or cannot be given the MCP set configured right now, fails as a send rather than as a turn that produced nothing. Lifecycle status is not a precondition of prompting; a session that has only been read attaches on its first message. That prompt also carries the model chosen while the session held no provider, applied against what the handshake itself reported — the picker for an unattached session offers the agent's own pre-session catalog, which is a list of what the agent can serve rather than what this session is running, so the choice is an intent until an attach can put it somewhere. A session already holding a provider reports its own options and is configured through `setSessionConfig` instead.
- An agent that does not support `session/load`, or that fails to restore the session, does not end the conversation: Ora builds a replacement provider session with `session/new` carrying the same Snapshot, and the next prompt carries the recorded transcript into it, exactly as an agent switch does. Nothing is written to history for this — the rebuilt binding stays out of the row until the prompt carrying the transcript is accepted, so a crash in between leaves the old binding in place and the following prompt rebuilds and re-delivers rather than continuing against an agent that holds nothing. Because that identity lives only in memory until then, every later frame for the session — prompt, cancel, close, MCP refresh, title polling — reads it from the actor rather than from the row.
...
- The incoming provider session is handshaken and optionally configured before the lifecycle lock is taken. Nothing is torn down until that succeeds, so an unavailable CLI leaves the conversation exactly where it was. Switching to the currently bound CLI reports `session_agent_unchanged`; degraded history reports `session_history_degraded`.
- Because rebinding stops the current actor, the client commits the move with the next message rather than when the agent is picked. Agent and model selections remain local until that send, and returning to the currently bound agent withdraws the pending move.
...
- The response carries the new handshake's `availableCommands` and `configOptions`, so the client immediately replaces the outgoing agent's capabilities with authoritative state from the incoming session.
...
Ora deletion removes Ora-owned database records and the session history Ora itself wrote, and registers durable Git cleanup jobs (executed asynchronously by the backend worker) for the deleted tasks' worktrees and `ora/*` branches. Provider-side history survives deletion on the Ora side. ACP `session/delete` is used only to roll back a provider session that failed before becoming visible; every user-visible session is closed, never deleted. Session deletion serializes against new actor operations, unloads its route, soft-deletes the row under the same lifecycle guard, and then removes the history file. Task and Project deletion reject Running descendants and transactionally cascade stopped Ora records; reading a conversation no longer makes it Running, so only sessions actually attached to a provider hold a delete off.
