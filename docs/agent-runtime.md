...
- Supervisors are keyed by that identity, which for a plugin is the package name its manifest declares, not the `namespace/name` address the package is installed and addressed under. Built-in CLIs claim theirs first, so a plugin package declaring an identity already supervised is ignored rather than allowed to replace the agent the user already had. The installed set comes from the plugin lifecycle, which is also the sole owner of plugin processes: a supervisor attaches to a running plugin instead of launching one, and asks the lifecycle to stop it when a generation ends.
- The supervised set is reconciled whenever the installed packages change, so installing a plugin makes its agent reachable in the running process and uninstalling one removes it. Enabling a plugin additionally retries its agent at once rather than at the end of the current backoff, because enabling is the moment a plugin that was refusing to attach becomes usable. Every installed plugin is supervised whether or not it is enabled; a disabled or uninstalled plugin refuses to attach and is reported exactly like a CLI that is not installed, so it is retried silently until the user enables it.
