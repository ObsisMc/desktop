...
`ora-backend` starts one independently supervised ACP connection for every installed [agent plugin](../crates/backend/src/agent_runtime/plugin_agent/README.md) when a Backend instance opens. Ora launches no agent CLI directly. Every persisted Ora Session owns a serialized actor, but actors targeting the same agent share its application-scoped ACP connection and route events by the private provider session id. One Session accepts only one load or prompt operation at a time while different Sessions remain concurrent.

## Process and Session Lifecycle

- Session contracts select an `agent_ref`: the provider plugin's namespaced package id, carried as an open string. Which agents exist depends on installed plugins and is not knowable when Ora is built, so an identity the runtime does not recognize means "that provider is not installed right now" — an ordinary runtime state reported as `agent_runtime_unavailable`, not corrupt data. OpenCode, Nga, CodeAgentCLI, Claude Code, and Codex keep their existing `ora-space.*` identities while their packages own all launch behavior.
- Supervisors are keyed by that identity. Discovery rejects duplicate package ids; the runtime defensively keeps only the first source for an identity.
- Each plugin owns executable resolution and provider-specific fallbacks. Nga and CodeAgentCLI search PATH before their official per-user install directories; the Claude and Codex ACP adapters use PATH or explicit environment-variable pins rather than assuming they live in the underlying CLI's data directory.
- `agent/start` receives the user's home directory as a neutral process cwd. Plugins launch and drain their child processes there, while per-session worktree paths travel through ACP `session/new` unchanged.
...
- Connection loss fails that agent's in-flight operations, marks only its registered Sessions Stopped, terminates and reaps the old plugin process tree, and only then starts a replacement. Sessions are loaded again only on demand; prompts are never replayed automatically.
...
- **A stored frontend agent preference is dropped when it cannot be offered.** `ora.settings.v1`
  ...
  or a plugin that has since been uninstalled — falls back to the product default on load. The
  choice is lost once; nothing else breaks.

Two limits are deliberate rather than incidental:

- **The chat model picker still offers only the five product-catalogued plugins.** Any other plugin-provided agent is
