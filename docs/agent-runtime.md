...
- Built-in executable resolution is repeated on every retry generation. Each directory on `PATH` is searched first (on Windows trying every `PATHEXT` extension), then the fixed per-user install directory is used as a fallback: `<home>/.nga/bin/nga` or `<home>/.codex/bin/codex-acp`. Agent plugins own their executable resolution; the CodeAgentCLI plugin preserves PATH-first lookup plus `<home>/.codeagentcli/bin/codeagentcli`, and supports an explicit `ORA_CODEAGENTCLI_BIN` pin.
- The shared child starts in the user's home directory with piped stdin, stdout, and stderr. Built-in Nga starts with `acp`, while `codex-acp` takes no arguments. The CodeAgentCLI plugin preserves the former built-in launch contract by starting `codeagentcli acp` in the user's home directory and forwarding per-session worktree paths through ACP. Stderr is drained continuously so provider diagnostics cannot block the process.
